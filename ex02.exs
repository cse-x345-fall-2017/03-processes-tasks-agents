...
defmodule Ex02 do
  ...
  def new_global_counter do
     Agent.start_link(fn -> 0 end, name: @globalAgent)    
  end
  
  def global_next_value do
     Agent.get_and_update(@globalAgent, fn state -> {state, state + 1} end)    
  end
...
defmodule Test do
  ...
  """
   ...
   test "higher level API interface" do
     {:ok, agent} = Ex02.new_counter(5)
     assert  Ex02.next_value(agent) == 5
     assert  Ex02.next_value(agent) == 6
   end
